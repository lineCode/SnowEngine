...
#include <set>
#include"../Mainloop/ShortProcess.h"
...
//---------------------------------------------------------------------------------------------------------------------
// Prototypes for the functions to export.                              - 
//---------------------------------------------------------------------------------------------------------------------
...
public:
	...
	static void LuaPrint(LuaPlus::LuaObject text, float lastTime = 3000);
...
void InternalScriptExports::LuaPrint(LuaPlus :: LuaObject text,float lastTime)
{
	if (!g_pApp) return;
	if (text.IsConvertibleToString())
	{
		std::string textToPrint = text.ToString();
		g_pApp->AddRenderToScreenText(textToPrint,lastTime);
	}
}
...
//将C++方法（一般是发送事件）注册到Lua环境中 
//方法封装在 InternalScriptExports 中
//调用时间:引擎初始化
void ScriptExports::Register(void)
{
	...
	globals.RegisterDirect("PrintToScreen", &InternalScriptExports::LuaPrint);
