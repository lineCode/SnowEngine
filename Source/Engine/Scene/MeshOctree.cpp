...
void MeshOctree::Build(const std::vector<XMFLOAT3>& vertices, const std::vector<UINT>& indices)
{
	...
	mRoot = new MeshOctreeNode();
	mRoot->Bounds = sceneBounds;

	BuildOctree(mRoot, indices);
}

bool MeshOctree::RayOctreeIntersect(FXMVECTOR rayPos, FXMVECTOR rayDir)
{
	return RayOctreeIntersect(mRoot, rayPos, rayDir);
}

XNA::AxisAlignedBox MeshOctree::BuildAABB()
{
...
void MeshOctree::BuildOctree(MeshOctreeNode* parent, const std::vector<UINT>& indices)
{
	...
	else
	{
		...
		for (int i = 0; i < 8; ++i)
		{
			...
			parent->Children[i] = new MeshOctreeNode();
...
bool MeshOctree::RayOctreeIntersect(MeshOctreeNode* parent, FXMVECTOR rayPos, FXMVECTOR rayDir)
{
