...
{
	friend class Engine;						// This is only to gain access to the view list
...
public:
	...
	bool isRunLua = true;
	const bool CanRunLua() const { return isRunLua&&(!IsProxy() || GetState()!=BGS_Running); }
    ...
    virtual StrongActorPtr VCreateActor(const std::string &actorResource, TiXmlElement *overrides, const Mat4x4* initialTransform=NULL, const ActorId serversActorId=INVALID_ACTOR_ID);  //  note: don't store this strong pointer outside of this class
